		...
		stage('DAST') {
		 steps {
			...
			sh 'docker run --rm -v $(pwd):/zap/wrk -u $(id -u ${USER}):$(id -g ${USER}) -t owasp/zap2docker-stable zap-baseline.py -t http://192.168.80.101:8080/webapp/ -g gen.conf -r basescan.html || true '
	                //docker run --rm -v $(pwd):/zap/wrk -u $(id -u ${USER}):$(id -g ${USER}) -t owasp/zap2docker-stable zap-full-scan.py -t http://192.168.80.101:8080/webapp/ -g gen.conf -r fullscan.html
			}
			}
